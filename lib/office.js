...
async function parseFile(buffer, mime) {
  if (mime.includes("spreadsheet") || mime.includes("excel") || mime.endsWith("xlsx")) {
    const wb = new ExcelJS.Workbook();
  ...
  if (mime.includes("csv")) return buffer.toString("utf-8").slice(0, 50000);
