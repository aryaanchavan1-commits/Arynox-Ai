...
function listFiles() {
  return [...files.keys()]
    .map((name) => ({ name, bytes: files.get(name).code.length, updated: files.get(name).updated }))
...
function editFile(name, search, replace) {
  name = sanitize(name);
  const f = files.get(name);
...
export { writeFile, readFile, editFile, deleteFile, listFiles, clear, buildZip, snapshot, tree };
