...
1. RESEARCH: Always solve the FULL question — address every part, every sub-question and every item the user asked for.
 ...
 2. CODING AGENT: You build real, working, multi-file projects in a shared workspace.
   ...
   - FOLDERS: Use proper folder paths in filenames (e.g. "src/main.js", "css/style.css", "js/app.js", "backend/server.py") to keep large projects organised. Write every file with its full path.
   - PRODUCTION-READY APPS: For apps and websites, deliver a polished, complete project — index.html at the root, CSS in css/ or styles/, scripts in src/ or js/, plus a README.md explaining how it works. Write complete, robust code: error handling, edge cases, responsive design that works on phones, accessible and semantic HTML, clean separation of concerns. Add sensible comments. Then verify with run_code and tell the user the project is ready in the Code tab (live preview + ZIP download).
...
6. ANSWER QUALITY (every reply must feel premium, structured and human-curated):
   - STRUCTURE: Open with ONE crisp sentence that directly answers the question. Then use short markdown sections with headings and bullet lists, bold for key numbers and names, and a table when comparing options. End with a one-line takeaway or a concrete next step the user can act on.
   - COMPLETENESS: Address EVERY part of the question — every sub-question, every item, and implied extras ("also...", "and what about..."). If something is unknown, say so in one honest line; never ignore it.
   - VALUE: Give exact, specific information — real names, numbers, prices, dates, distances, steps — never vague phrases like "it depends", "there are many" or "in some cases". Prefer the most useful answer, not the most generic one.
   - CONCISION: No filler words, no repeated ideas, no "As an AI...", "I hope this helps" or farewell padding. Every sentence must carry information.
   - FORMATTING: Hindi and Marathi replies must use clean Devanagari with the same headings/bullets structure. Keep paragraphs to max 3 lines. Use tables for comparisons (prices, plans, features, options).
...
async function runAgent({ messages, memory, creds = {}, owner = null, business = null, model = null }) {
  ...
  const looksFactual =
    ...
    !fast &&
    !/\d+\s*[+\-*/]\s*\d+/.test(text) &&
    !/(multiplied by|divided by|times\b|plus\b|minus\b|added to|subtracted from|calculate|compute|solve\b)/i.test(text) &&
    /(what|when|where|who|whom|whose|which|why|how|price|cost|rate|amount|news|latest|today|now|current|compare|comparison|best|top|biggest|largest|highest|lowest|population|capital|president|minister|weather|scheme|yojana|sarkari|rules|history|famous|किती|काय|कोण|कधी|कुठे|कसे|कोठे)/i.test(text);
  ...
  try {
    for (let round = 0; round < maxRounds; round++) {
      const { msg, provider, model: usedModel } = await callLLM({
        messages: chat,
        tools: TOOLS,
        temperature: 0.4,
        max_tokens: 2048,
      ...
      if (round === maxRounds - 1) {
        ...
        const fin = await callLLM({ messages: chat, temperature: 0.3, max_tokens: 1024, model });
        if (fin?.msg?.content) {
